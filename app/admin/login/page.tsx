    ...
    <main className="min-h-screen flex items-center justify-center p-6 bg-slate-900">
      ...
      >
        <div className="text-center mb-10">
          <h1 className="text-4xl font-light tracking-tight text-white mb-2 tracking-widest uppercase">Admin Portal</h1>
          <p className="text-white/40 italic text-xs uppercase tracking-[0.3em]">Sacred space for writers</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          {error && (
            <p className="text-red-400 text-center text-xs font-mono uppercase tracking-widest">{error}</p>
          )}
        ...
        <div className="mt-10 pt-8 border-t border-white/5 text-center">
          <Link href="/" className="text-[10px] text-white/30 hover:text-amber-400 transition-colors uppercase tracking-[0.3em]">← Return to Sanctuary</Link>
        </div>
      </motion.div>
    </main>
  );
