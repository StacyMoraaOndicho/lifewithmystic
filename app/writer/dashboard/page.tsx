...
function DashboardContent() {
  ...
  // Polling logic: If they just paid, check status every 2 seconds
  useEffect(() => {
    if (mustPay && searchParams.get('status') === 'success' && pollCount < 5) {
  ...
  }, [mustPay, pollCount, searchParams]);

  async function checkSubscriptionAndFetchData() {
    try {
      // Force fresh fetch to bypass caching
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center text-white">
      <div className="text-center">
        ...
        <p className="font-mono text-[10px] uppercase tracking-[0.5em] animate-pulse">Entering Sanctuary...</p>
      </div>
    </div>
  );
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6 bg-[#0a0a0a]">
        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-[#0a0a0a] border border-white/10 w-full max-w-lg rounded-[40px] p-12 text-center shadow-3xl">
          ...
          <p className="text-white/40 text-[10px] uppercase tracking-widest mb-12">
            {searchParams.get('status') === 'success' ? 'Verifying payment... please wait' : 'Choose your gateway to proceed as a writer'}
          </p>
          ...
          <p className="mt-10 text-[9px] text-white/20 uppercase tracking-[0.3em]">Payable once to unlock all tools</p>
        </motion.div>
      </main>
    );
  }
...
export default function WriterDashboard() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center text-white font-mono uppercase tracking-[0.5em] text-[10px]">Initializing Sanctuary...</div>}>
      <ProtectedRoute>
        <DashboardContent />
      </ProtectedRoute>
    </Suspense>
