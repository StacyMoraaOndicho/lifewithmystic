    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6 bg-[#0a0a0a]">
        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-[#0a0a0a] border border-white/10 w-full max-w-lg rounded-[40px] p-12 text-center shadow-3xl">
          <h2 className="text-3xl font-light text-white mb-2 uppercase tracking-[0.2em]">Activate Sanctuary</h2>
          <p className="text-white/40 text-[10px] uppercase tracking-widest mb-12">Select your gateway to proceed as a writer</p>
          <div className="space-y-4 text-left">
            {paymentMethods.map((m) => (
              <button key={m.id} onClick={() => handlePayment(m.id)} disabled={!!payLoading} className="w-full p-6 rounded-3xl bg-white/[0.03] border border-white/5 hover:border-white/20 transition-all flex items-center justify-between">
                <div className="flex items-center gap-5">
                  <div className="p-3 rounded-2xl bg-white/5">{m.icon}</div>
                  <div><h4 className="text-white font-medium text-sm">{m.name}</h4><p className="text-[9px] text-white/30 mt-1">{m.tag}</p></div>
                </div>
                {payLoading === m.id ? <Loader2 className="animate-spin w-4 h-4" /> : <ChevronRight className="w-4 h-4 text-white/10" />}
              </button>
            ))}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {products.length > 0 ? products.map((p) => (
                  <div key={p.id} className="p-10 rounded-[40px] border border-white/10 bg-white/[0.02] flex items-center justify-between">
                  ...
                  <Link href="/writer/settings" className="block col-span-full">
                    <button className="w-full p-16 rounded-[40px] border-2 border-dashed border-white/10 text-white/20 hover:border-[var(--accent)]/30 transition-all flex flex-col items-center justify-center gap-4 group">
              ...
              <div className="rounded-[40px] border border-white/10 bg-white/[0.01] overflow-hidden shadow-2xl">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-white/[0.05]">
                    {[{ title: 'Silence as Mirror', views: '450', status: 'Published' }, { title: 'The Divine Within', views: '320', status: 'Published' }].map((post) => (
                      <tr key={post.title} className="hover:bg-white/[0.02] transition-colors"><td className="px-10 py-8 font-light text-white text-base">{post.title}</td><td className="px-10 py-8 text-white/40 font-mono text-base">{post.views}</td><td className="px-10 py-8 text-right"><span className="text-emerald-400/60 text-[10px] uppercase font-bold tracking-widest bg-emerald-400/5 px-3 py-1 rounded-full">{post.status}</span></td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
          ...
          <div className="space-y-12">
            <section className="p-12 rounded-[50px] bg-[var(--accent)] text-[var(--bg)] relative overflow-hidden group shadow-3xl">
              ...
              <p className="text-[var(--bg)]/70 text-sm mb-10 leading-relaxed italic font-light">Your current resonance is up 20% this week. Share your latest reflection to keep the momentum flowing.</p>
              <button className="w-full py-5 bg-[var(--bg)] text-[var(--accent)] rounded-2xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-3">Radiate Now</button>
            </section>
...
export default function WriterDashboard() {
  return (
    <ProtectedRoute>
      <Suspense fallback={<div className="min-h-screen bg-black flex items-center justify-center text-white">Loading...</div>}><DashboardContent /></Suspense>
    </ProtectedRoute>
  );
}
