...
function DashboardContent() {
  ...
  async function checkSubscriptionAndFetchData() {
    try {
      ...
      // STRICT CHECK: Does the database show this user as 'active'?
      ...
      <div className="max-w-6xl mx-auto pt-16 font-light">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          ...
          <div className="flex gap-4">
            <Link href="/studio"><button className="px-8 py-4 bg-[var(--accent)] text-[var(--bg)] rounded-2xl font-bold uppercase tracking-widest text-[10px] shadow-2xl hover:shadow-[var(--accent)]/30 transition-all">New Reflection</button></Link>
          ...
          {[
            ...
            <div key={stat.label} className="p-10 rounded-[40px] border border-white/5 bg-white/[0.01] hover:bg-white/[0.02] transition-all group shadow-sm">
              <div className="flex items-center gap-4 mb-6">
                <div className={`p-3 rounded-lg bg-[var(--text)]/5 ${stat.color} group-hover:scale-110 transition-transform`}>{stat.icon}</div>
